...
| `dcli-mcp` | **`dx-mcp` MCP 서버** — AI 에이전트가 직접 조작(rmcp/stdio), CLI와 동일 엔진 |
...
### `dx-mcp` MCP 서버 (Phase 4)

AI 에이전트가 직접 조작하는 stdio MCP 서버. CLI verb와 **같은 dispatch 엔진**을 쓴다.
12개 tool: doc_open/create/info/list/close/snapshot, layer_list/get, batch_apply,
export_png, history_undo/redo.

- **batch_apply가 주 쓰기 경로** — Action 배열을 트랜잭션으로(전부 성공 또는 전체 롤백),
  `bind`로 신규 노드를 같은 batch 내에서 참조(ID를 발명하지 않음).
- **doc_snapshot**이 합성 결과를 base64 PNG로 반환 → 에이전트가 결과를 "본다"(인지 루프).
- 에러 2분기: 검증/op 실패는 `isError:true`+구조화 이슈(self-correction), 미지 tool만
  JSON-RPC error.
- 단일 편집기 락(`.dxdoc/.lock`)으로 CLI/studio/MCP 동시 오픈 배타화.
...
~~P1 문서모델~~ → ~~P2 CLI verb(`dx`)~~ → ~~P3 네이티브 셸~~ → ~~P4 MCP 툴(`dx-mcp`)~~ →
**P5 PSD Tier0** → P6 브라우저 셸 → P7 충실도 확장.
